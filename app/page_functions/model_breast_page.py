...
def breast_page():
    ...
    for row in range(num_rows):
        ...
        for col_index, f_index in enumerate(range(row * num_cols, min((row + 1) * num_cols, num_features))):
            ...
            label_width = 200  # Defina o tamanho fixo para o rótulo
            value = cols[col_index].slider(f"{f_name}: ", min_value=df[f_name].min(), max_value=df[f_name].max(), key=f_name, width=label_width)
            data.append(value)
